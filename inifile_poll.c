...
#define INIFILE_MEMORY_POOL_SIZE	1024
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/*
...
static MPL_POOL_S	*global_pool = NULL;
...
void* malloc_memset(MPL_POOL_S *pool,size_t size)
{
	void *p = NULL;

	p = mpl_palloc(pool,size);
	if(p)
	{
		memset(p,0,size);
	}else{
...
int init_line_key_val(char *line,INI_FILE_S *p_file)
{
	...
	if(p_equ && p < p_equ)
	{
		...
		len = pp - p + 2;
		p_key = (char *)malloc_memset(p_file->pool,len);
		if(p_key == NULL)
		{
		...
		len = pp - p + 2;
		p_value = (char *)malloc_memset(p_file->pool,len);
		if(p_value == NULL)
		{
			return INIFILE_SYSTEM_ERROR;
		}
		...
		pk = (KEY_VALUE_NODE *)malloc_memset(p_file->pool,sizeof(KEY_VALUE_NODE));
		if(pk == NULL)
		{
			return INIFILE_SYSTEM_ERROR;
		}
...
int init_line_section(char *line,INI_FILE_S *p)
{
	...
	if(p_brack && q_brack)
	{
		...
		if(p_brack < q_brack)
		{
			...
			ps = (char *)malloc_memset(p->pool,len);
			if(ps == NULL)
				return INIFILE_SYSTEM_ERROR;
			len--;
			...
			q = (FILE_SECTION_NODE *)malloc_memset(p->pool,sizeof(FILE_SECTION_NODE));
			if(q == NULL)
			{
...
int init_ini_file(const char *filename,int len)
{
	...
	if(global_pool == NULL)
		global_pool = mpl_create_pool(INIFILE_MEMORY_POOL_SIZE);
	if(global_pool == NULL)
	{
		ret = INIFILE_SYSTEM_ERROR;
		goto error_out;
	...
	file_name = (char *)malloc_memset(global_pool,name_len);
	if(file_name == NULL)
	{
		ret = INIFILE_SYSTEM_ERROR;
		goto error_out;
	...
	if(p_file == NULL)
	{
		p_file = (INI_FILE_S *)malloc_memset(global_pool,sizeof(INI_FILE_S));
		if(p_file == NULL)
		{
			ret = INIFILE_SYSTEM_ERROR;
			goto error_out;
	...
	p_file->pool = mpl_create_pool(INIFILE_MEMORY_POOL_SIZE);
	if(p_file->pool == NULL)
	{
		ret = INIFILE_SYSTEM_ERROR;
		goto error_out;
...
int update_value_ofkey(int ini_fd,INI_PARAMETER *parameter)
{
	...
	p_value = (char *)malloc_memset(p->pool,len);
	if(p_value == NULL)
	{
		ret = INIFILE_SYSTEM_ERROR;
		goto error_out;
...
int add_value_ofkey(int ini_fd,INI_PARAMETER *parameter)
{
	...
	p_key = (char *)malloc_memset(p->pool,len);
	if(p_key == NULL)
	{
		ret = INIFILE_SYSTEM_ERROR;
		goto error_out;
	...
	pk = (KEY_VALUE_NODE *)malloc_memset(p->pool,sizeof(KEY_VALUE_NODE));
	if(pk == NULL)
	{
		ret = INIFILE_SYSTEM_ERROR;
		goto error_out;
...
int add_ini_section(int ini_fd,INI_PARAMETER *parameter)
{
	...
	p_section = (char *)malloc_memset(p->pool,len);
	if(p_section == NULL)
	{
		ret = INIFILE_SYSTEM_ERROR;
		goto error_out;
